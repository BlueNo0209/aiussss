...
#include "RS485_Oriental_Motor.h"
#include "xmlReadv2.h"
#include "map_maker.h"
#include "EthernetRemoteMotor.h"
//#include "GPIO.h"
...
double acc = 1000, spd = 1000,turnRate = 0.2;
int Motor_PEMS = 100, Motor_PEVAL = 1000;
int Motor_CEMS = 500, Motor_CEVAL = 500;
int Motor_ENAFC = 0;
...
//RS485_TECO_Motor::Motor_Control Motor(SOWAN_PARA.RS485Port);
RS485_Oriental_Motor::Motor_Control Motor;
...
Remote_Motor_Server EtherServer;
ecl::Thread GUI_Ether_Thread;

void GUI_ETHER_RECV(void) {
	//GUI_Serv.GUI_SERVER_INIT();
	//GUI_Serv.ether_main();

	printf("ThreadEtherComm\n");
	EtherServer.Socket_init();
	printf("Socket_init\n");
	EtherServer.ether_main();
}
...
void MotorMoveStop(double acc) {
	Motor.Motor_group_stop();
	//Motor.JOG_Acc_Setting(acc, 1);
	//Motor.JOG_Acc_Setting(acc, 2);
	//Motor.Motor_Move_Program_AccSet(acc, acc, MotorID(1));
	//Motor.Motor_Move_Program_AccSet(acc, acc, MotorID(2));
};
void MotorMoveStop() {
	MotorMoveStop(1500);
};
...
void SaveMapFunction() {
	...
	if (GetSaveCmd) {
		std::string mapname_ = "/home/pi/catkin_ws/src/hiroshima_patrol/map/MapAutoSave";
...
int main(int argc, char **argv)
{
  ...
  GUI_Ether_Thread.start(GUI_ETHER_RECV);
...
void main_init() {
	...
	Motor.Motor_Init(SOWAN_PARA.RS485Port);

	Motor.Motor_Setting(MOTOR_TYPE_AR, MotorID(1));
	Motor.Motor_Setting(MOTOR_TYPE_AR, MotorID(2));
	...
	RobotPosInfo.initial(Motor.Motor_Pos_Return(1), Motor.Motor_Pos_Return(2), 0, 0, 0);
	RobotOdomInfo.initial(Motor.Motor_Pos_Return(1), Motor.Motor_Pos_Return(2), 0, 0, 0);
...
void main_function() {
	...
	if (MotorMovCmdPre != MotorMovCmd) {
		if (MotorMovCmd != 0) {
			User_GUI_control(MotorMovCmd);
		}
		else {
			MotorMoveStop();
		}
...
void GUI_control(char ttt) {
	...
	case '1':
		Motor.Motor_Stop(MotorID(1));
		Motor.Motor_Stop(MotorID(2));
		break;
	case '2':
		Motor.Motor_Free(MotorID(1));
		Motor.Motor_Free(MotorID(2));
		break;
	case '3':
		spd += 500;
		break;
...
void MotorMoveControl(char dir ) {
	...
	switch (dir) {
	case 'x':
		Motor.RUN_Input_Group(spd, 'B');
		break;
	case 'w':
		Motor.RUN_Input_Group(spd, 'F');
		break;
	case 'd':
		Motor.RUN_Input_Group(spd/2, 'L');
		break;
	case 'a':
		Motor.RUN_Input_Group(spd/2, 'R');
		break;
	case 's':
		Motor.Motor_group_stop();
		break;
	...
	case 'e':
		break;
	case 'b':
		//Motor.RUN_Input(spd_b, MOTOR_MOVE_REV, MotorID(1));
		//Motor.RUN_Input(spd_b, MOTOR_MOVE_FWD, MotorID(2));
		break;
	default:
		dir = 's';
	}
...
void GUI_ethernet_Input() {
	...
	/*cv::Point RobotImgPos = TranFunc.Real2Img(TranFunc.TF_Pos.Pos) + cv::Point(Map_Rect.x, Map_Rect.y);
	cv::Point RobotTagPos = TranFunc.Real2Img(MovTag_Save) + cv::Point(Map_Rect.x, Map_Rect.y);*/
	//GUI_Serv.RobotData.robot_x = RobotImgPos.x;
	//GUI_Serv.RobotData.robot_y = RobotImgPos.y;
	//GUI_Serv.RobotData.robot_x_tag = RobotTagPos.x;
	//GUI_Serv.RobotData.robot_y_tag = RobotTagPos.y;


	int tmp_spd;
	RECV_CMD_LIST_TYPE tmp_CMD_Data;
	MOTOR_LOCAL_SETTING* tmp_LocalSet = &EtherServer.MotorLocalSetting[tmp_CMD_Data.ID];
	bool tmp_Recv_New_CMD = EtherServer.Recv_New_CMD();

	while (tmp_Recv_New_CMD) {
		tmp_CMD_Data = EtherServer.Recv_Cmd_RETURN();
		printf("ID[%d],[%s,%d]\n", tmp_CMD_Data.ID, EtherMotorCmdString[tmp_CMD_Data.CMD].c_str(), tmp_CMD_Data.Value);

		switch (tmp_CMD_Data.CMD) {
		...
		case EtherMotorCmd::SPD_SET:
			spd = tmp_CMD_Data.Value / 2;
			break;
		case EtherMotorCmd::MACRO_PGM:
			break;
		case EtherMotorCmd::SET_STAMP:
			EtherServer.UpdateMotorMotionStamp(tmp_CMD_Data.ID, tmp_CMD_Data.Value);
			break;
		case EtherMotorCmd::SET_STAMP_MACRO:
			EtherServer.UpdateMacroDateStamp(tmp_CMD_Data.ID, tmp_CMD_Data.Value);
			break;
		default:
			break;
		}
		tmp_Recv_New_CMD = EtherServer.Recv_New_CMD();
	}
...
void SelfPosUpdate(bool speed_on, double time) {
	...
	p1 = Motor.Motor_Pos_Return(1);
	p2 = Motor.Motor_Pos_Return(2);
	if (speed_on) {
...
void Para_2_Img() {
	...
	Drew_IMG(&img, "Pos_1", std::to_string(Motor.Motor_Pos_Return(1)), cv::Point(setposxString, setposy));
	Drew_IMG(&img, "Pos_2", std::to_string(Motor.Motor_Pos_Return(2)), cv::Point(setposxString + 200, setposy));
	setposy += 20;
	Drew_IMG(&img, "RPM_1", std::to_string(Motor.Motor_Speed_Return(1)), cv::Point(setposxString, setposy));
	Drew_IMG(&img, "RPM_2", std::to_string(Motor.Motor_Speed_Return(2)), cv::Point(setposxString + 200, setposy));
	setposy += 20;
	Drew_IMG(&img, "State_1", std::to_string(Motor.Motor_State_Return(1)), cv::Point(setposxString, setposy));
	Drew_IMG(&img, "State_2", std::to_string(Motor.Motor_State_Return(2)), cv::Point(setposxString + 200, setposy));
	setposy += 20;
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan) {
	...
	for (int i = 0; i < count; i++) {
		...
		angle = scan->angle_min + scan->angle_increment * i + DEG_OFFDET;
		rawData = scan->ranges[i];
